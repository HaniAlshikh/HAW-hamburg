...
class MyHuffmanTest {
    ...
    void calculateCodeFromHuffmanTree(String dataStr, byte[] byteArr, int[] freqArr, String[] codeArr) throws IOException {
        ...
        for (int i = 0; i < codeArr.length; i++) {
            ...
            assertEquals(current.byt, byteArr[i]);
            assertEquals(current.freq, freqArr[i]);
    ...
    @ParameterizedTest
    @MethodSource("provideTestCases")
    void encodeDataDebug(String dataStr) throws IOException {
        data = new ByteArrayInputStream(dataStr.getBytes());
        huffman.setEncodingStreams(data, encodedDataOutput);
        huffman.encode();

        // for debugging
        //printHuffmanTreeBits(huffman.root);
        //printBits(31, huffman.data.length);
        //printBytesCodes(dataStr.getBytes(), huffman.codeTable);
        //System.out.println();
        //
        //byte[] writtenBytes = Files.readAllBytes(Path.of(binFile));
        //writeByteArr(writtenBytes);
    }
    ...
    void encodeDecodeData(String dataStr) throws IOException {
        data = new ByteArrayInputStream(dataStr.getBytes());
        MyHuffman.encode(data, encodedDataOutput);
        MyHuffman.decode(encodedData, decodedDataOutput);
    ...
    public void wasItWorthIt(String fileName) throws IOException {
        ...
        MyHuffman.encode(new FileInputStream(file), new FileOutputStream(fileCompressed));
        ...
        MyHuffman.decode(new FileInputStream(fileCompressed), new FileOutputStream(fileDecompressed));
    ...
    private static Stream<Arguments> provideTestCases() {
        ...
        for (String[][] testCase : testCases) {
            byte[] charArr = Arrays.stream(testCase[1]).reduce(String::concat).orElseThrow().getBytes();
            int[] freqArr = Arrays.stream(testCase[2]).mapToInt(Integer::valueOf).toArray();
            arguments.add(Arguments.of(testCase[0][0], charArr, freqArr, testCase[3]));
        }

        return arguments.stream();
    }

    private void printHuffmanTreeBits(HuffmanNode node) {
        if (node.isLeaf()) {
            System.out.print(1);
            printBits(8, node.byt);
        } else {
            System.out.print(0);
            printHuffmanTreeBits(node.left);
            printHuffmanTreeBits(node.right);
        }
    }

    private void printBits(int size, int data) {
        ArrayDeque<String> queue = new ArrayDeque<>();
        for (int i = 0; i < size; i++)
            queue.addFirst(((((byte) data) & (1 << i)) != 0) ? "1" : "0");
        queue.forEach(System.out::print);
    }

    public static void printBytesCodes(byte[] bytes, Map<Byte, Queue<Boolean>> codeTable) {
        // ABC -> 0011001 according to codeTable
        for (byte b : bytes) {
            codeTable.get(b).forEach(bool -> System.out.print(bool ? "1" : "0"));
        }
    }

    private static void writeByteArr(byte[] bytes) {
