...
public class Main {
    ...
    private static final int N_INCREMENT    =    10000;
    // Dies ist die obere Grenze der Ausführung
    private static final int N_END          = 10000000;
    ...
    private static final IntConsumer stringConcat = (iCurrentN) -> {
        String s = "";
        for (int index = 0; index < iCurrentN; ++index) {
            s += 'a';
        }};

    private static final IntConsumer stringBuilder = (iCurrentN) -> {
        StringBuilder s = new StringBuilder();
        for (int index = 0; index < iCurrentN; ++index) {
            s.append('a');
        }};

    private static final IntConsumer linkedList = (iCurrentN) -> {
        LinkedList<String> s = new LinkedList<>();
        for (int index = 0; index < iCurrentN; ++index) {
            s.add("a");
        }};

    private static final IntConsumer arrayList = (iCurrentN) -> {
