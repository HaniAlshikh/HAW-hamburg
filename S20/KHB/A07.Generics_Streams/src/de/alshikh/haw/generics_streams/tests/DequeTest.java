package de.alshikh.haw.generics_streams.tests;
...
class DequeTest {
    ...
    @BeforeEach
    void setUp() {
        intDeque = new Deque<>(capacity);
        strDeque = new Deque<>(capacity);
        empty = new Deque<>(0);
    ...
    void enqueue() {
        ...
        assertDoesNotThrow(() -> strDeque.enqueue("new first"));
        assertEquals("new first", strDeque.getFirst());
    }
    ...
    @Test
    void grow() {
        assertEquals(capacity, intDeque.size());
